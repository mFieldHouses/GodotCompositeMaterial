#This script was generated using addons/CompositeMaterial/tools/shader_composition_strings_creator.tscn.
...
const parameters_string : String = '
group_uniforms layer_%s;
/**
 * Whether this layer will display at all.
 */
uniform bool layer_%s_enabled = true;
/**
 ...
 */
uniform bool layer_%s_propagate_normals = false;
group_uniforms layer_%s.uv1;
uniform int layer_%s_uv1_mode : hint_enum("From Mesh", "Triplanar");
uniform int layer_%s_uv1_triplanar_mode : hint_enum("Local", "Global");
/**
 * X and Y components by which the size of this UV map will be multiplied.
 */
uniform vec2 layer_%s_uv1_scale = vec2(1.0);
uniform vec2 layer_%s_uv1_offset = vec2(0.0);
/**
 * The Red and Green channels of this map are interpreted as U and V offsets respectively.
 */
uniform sampler2D layer_%s_uv1_offset_map : hint_default_transparent;
uniform vec2 layer_%s_uv1_offset_map_scale = vec2(1.0, 1.0);
/**
 * Number by which the UV offsets generated by the UV offset map will be multiplied before getting added to the final UVs.
 */
uniform float layer_%s_uv1_offset_map_factor = 1.0;
group_uniforms layer_%s.uv2;
uniform int layer_%s_uv2_mode : hint_enum("From Mesh", "Triplanar");
uniform int layer_%s_uv2_triplanar_mode : hint_enum("Local", "Global");
/**
 * X and Y components by which the size of this UV map will be multiplied.
 */
uniform vec2 layer_%s_uv2_scale = vec2(1.0);
uniform vec2 layer_%s_uv2_offset = vec2(0.0);
/**
 * The Red and Green channels of this map are interpreted as U and V offsets respectively.
 */
uniform sampler2D layer_%s_uv2_offset_map : hint_default_transparent;
uniform vec2 layer_%s_uv2_offset_map_scale = vec2(1.0, 1.0);
/**
 * Number by which the UV offsets generated by the UV offset map will be multiplied before getting added to the final UVs.
 */
uniform float layer_%s_uv2_offset_map_factor = 1.0;
group_uniforms layer_%s.uv_map_assignment;
uniform int layer_%s_normal_map_UV_assignment : hint_enum("UV1", "UV2") = 0;
uniform int layer_%s_albedo_map_UV_assignment : hint_enum("UV1", "UV2") = 0;
uniform int layer_%s_occlusion_map_UV_assignment : hint_enum("UV1", "UV2") = 0;
uniform int layer_%s_roughness_map_UV_assignment : hint_enum("UV1", "UV2") = 0;
uniform int layer_%s_metallic_map_UV_assignment : hint_enum("UV1", "UV2") = 0;
uniform int layer_%s_UV_offset_map_UV_assignment : hint_enum("UV1", "UV2") = 0;
uniform int layer_%s_texture_mask_A_UV_assignment : hint_enum("UV1", "UV2") = 1;
uniform int layer_%s_texture_mask_B_UV_assignment : hint_enum("UV1", "UV2") = 1;
uniform int layer_%s_UV_mask_UV_assignment : hint_enum("UV1", "UV2") = 0;
group_uniforms layer_%s.ORM;
uniform int layer_%s_orm_mode : hint_enum("Single map", "Seperate maps", "Values") = 0;
uniform sampler2D layer_%s_orm_map : hint_default_transparent;
uniform sampler2D layer_%s_occlusion_map : hint_default_transparent;
uniform sampler2D layer_%s_roughness_map : hint_default_transparent;
uniform sampler2D layer_%s_metallic_map : hint_default_transparent;
uniform lowp float layer_%s_occlusion_value : hint_range(0.0, 1.0, 0.001) = 0.5;
uniform lowp float layer_%s_roughness_value : hint_range(0.0, 1.0, 0.001) = 0.5;
uniform lowp float layer_%s_metallic_value : hint_range(0.0, 1.0, 0.001) = 0.5;
/**
 * Whether this layer should use the same ORM texture as the underlying layer.
 */
group_uniforms layer_%s.masking;
/**
 * Whether this layer should display only its masks in black and white.
 */
uniform bool layer_%s_consolidate_masks = false;
/**
 * Number by which the intensity of the resulting mask (A, B and Directional) will be multiplied.
 */
uniform float layer_%s_mask_amplification : hint_range(0.0, 10.0) = 1.0;
uniform int layer_%s_step_2_mixing_operation : hint_enum("Add", "Subtract", "Multiply") = 0;
uniform float layer_%s_step_2_mixing_threshold : hint_range(0.0, 1.0) = 0.0;
uniform int layer_%s_step_3_mixing_operation : hint_enum("Add", "Subtract", "Multiply") = 0;
uniform float layer_%s_step_3_mixing_threshold : hint_range(0.0, 1.0) = 0.0;
uniform int layer_%s_step_4_mixing_operation : hint_enum("Add", "Subtract", "Multiply") = 0;
uniform float layer_%s_step_4_mixing_threshold : hint_range(0.0, 1.0) = 0.0;
uniform int layer_%s_step_5_mixing_operation : hint_enum("Add", "Subtract", "Multiply") = 0;
...
uniform float layer_%s_post_effect_parameter_3 = 0.0;
 ...
 */
uniform int layer_%s_texture_masks_mix_operation: hint_enum("Add", "Subtract", "Multiply") = 0;
/**
 * In what order masks A and B will be substracted from one another when the mix mode is set to "subtract".
 */
uniform int layer_%s_texture_masks_subtraction_order: hint_enum("A - B", "B - A") = 0;
uniform int layer_%s_texture_masks_mixing_step: hint_enum("Step 1:1", "Step 2:2", "Step 3:3", "Step 4:4", "Step 5:5", "Step 6:6") = 1;
group_uniforms layer_%s.directional_mask;
/**
 ...
 */
uniform float layer_%s_positional_mask_max = 1.0;
/**
 * Position on the axis chosen starting at which the mask is 0. Doesnt apply to the Local positional mask mode.
 */
uniform float layer_%s_positional_mask_min = -1.0;
/**
 * What operation will be executed to mix masks ABD (already mixed) and the Positional Mask together.
 */
uniform int layer_%s_positional_mask_mixing_step: hint_enum("Step 1:1", "Step 2:2", "Step 3:3", "Step 4:4", "Step 5:5", "Step 6:6") = 3;
group_uniforms layer_%s.vertex_color_mask;
/**
...
const fragment_snippet_string : String = '
if (layer_%s_enabled == true) {
vec3 uv1_triplanar_pos;
vec3 uv1_triplanar_normal;
if (get_layer_uv_triplanar_mode(%s, 0) == 0) {
...
vec2 uv1;
if (get_layer_uv_mode(%s, 0) == 0) uv1 = get_layer_uv(%s, 0, UV);
else uv1 = get_layer_triplanar_uv(%s, 0, uv1_triplanar_pos, uv1_triplanar_normal, UV);
vec3 uv2_triplanar_pos;
vec3 uv2_triplanar_normal;
if (get_layer_uv_triplanar_mode(%s, 1) == 0) {
...
vec2 uv2;
if (get_layer_uv_mode(%s, 1) == 0) uv2 = get_layer_uv(%s, 0, UV);
else uv2 = get_layer_triplanar_uv(%s, 1, uv2_triplanar_pos, uv2_triplanar_normal, UV);
...
vec4 layer_%s_albedo_texture = textureLod(layer_%s_albedo, uv1 * float(layer_%s_albedo_uv_map_index == 0) + uv2 * float(layer_%s_albedo_uv_map_index == 1), layer_%s_lod_level + mip_modifier);
vec2 layer_%s_occlusion_factor;
vec2 layer_%s_roughness_factor;
vec2 layer_%s_metallic_factor;
switch (layer_%s_orm_mode) {
case 0:
vec4 orm_color = textureLod(layer_%s_orm_map, uv1 * float(layer_%s_occlusion_uv_map_index == 0) + uv2 * float(layer_%s_occlusion_uv_map_index == 1), layer_%s_lod_level + mip_modifier);
layer_%s_occlusion_factor = orm_color.ra;
layer_%s_roughness_factor = orm_color.ga;
layer_%s_metallic_factor = orm_color.ba;
break;
case 1:
layer_%s_occlusion_factor = textureLod(layer_%s_occlusion_map, uv1 * float(layer_%s_occlusion_uv_map_index == 0) + uv2 * float(layer_%s_occlusion_uv_map_index == 1), layer_%s_lod_level + mip_modifier).ra;
layer_%s_roughness_factor = textureLod(layer_%s_roughness_map, uv1 * float(layer_%s_roughness_uv_map_index == 0) + uv2 * float(layer_%s_roughness_uv_map_index == 1), layer_%s_lod_level + mip_modifier).ra;
layer_%s_metallic_factor = textureLod(layer_%s_metallic_map, uv1 * float(layer_%s_metallic_uv_map_index == 0) + uv2 * float(layer_%s_metallic_uv_map_index == 1), layer_%s_lod_level + mip_modifier).ra;
break;
default:
layer_%s_occlusion_factor = vec2(layer_%s_occlusion_value, 1.0);
layer_%s_roughness_factor = vec2(layer_%s_roughness_value, 1.0);
layer_%s_metallic_factor = vec2(layer_%s_metallic_value, 1.0);
};
vec3 layer_%s_normal_map = textureLod(layer_%s_normal, uv1 * float(layer_%s_normal_uv_map_index == 0) + uv2 * float(layer_%s_normal_uv_map_index == 1), layer_%s_lod_level + mip_modifier).rgb;
...
normal_mix = mix(normal_mix, layer_%s_normal_map, layer_%s_mask_factor * (%s.0 - float(layer_%s_propagate_normals)));
}'
...
uniform sampler2D vertex_displacement_map : hint_default_black; %parameters 
...
return get_fragment_distance_to_camera() / 20.0;
}
bool get_layer_enabled(int layer) {
%cr_get_layer_enabled_cr_
}
int get_layer_uv_mode(int layer, int uv_map_index) {
%cr_get_layer_uv_mode_cr_
}
...
vec2 get_layer_triplanar_uv(int layer, int uv_map_index, vec3 pos, vec3 normal, vec2 uv) {normal = abs(normal);
int direction = 0;
...
%cr_get_layer_post_effect_parameter_cr_
}
...
%cr_get_layer_texture_masks_subtraction_order_cr_
}
int get_layer_texture_mask_mix_operation(int layer) {
%cr_get_layer_texture_mask_mix_operation_cr_
}
...
%cr_get_layer_directional_mask_space_cr_
}
...
%cr_get_layer_positional_mask_axis_cr_
}
float get_layer_positional_mask_min(int layer) {
%cr_get_layer_positional_mask_min_cr_
}
float get_layer_positional_mask_max(int layer) {
%cr_get_layer_positional_mask_max_cr_
}
...
%cr_get_layer_uv_mask_mixing_operation_cr_
}
int get_layer_uv_mask_mixing_order(int layer) {
%cr_get_layer_uv_mask_mixing_order_cr_
}
...
%cr_get_layer_uv_mask_max_cr_
}
float get_texture_mask_for_layer(int layer, vec2 mask_a_uv, vec2 mask_b_uv) {float texture_mask_mix;
...
float get_directional_mask_for_layer(int layer, vec3 local_surface_normal, vec3 global_surface_normal) {vec3 surface_normal = local_surface_normal * float(get_layer_directional_mask_space(layer) == 1) + global_surface_normal * float(get_layer_directional_mask_space(layer) == 0);
...
float get_positional_mask_for_layer(int layer, vec3 local_v_position, vec3 global_v_position) {float positional_mask_value;
...
float get_vertex_color_mask_for_layer(int layer) {switch (get_layer_vertex_color_mask_mode(layer)) {
case 0:
return 0.0;
case 1:
...
float get_layer_mask_step_mix(int layer, int step, vec2 uv1, vec2 uv2, vec3 local_surface_normal, vec3 global_surface_normal, vec3 local_vertex_position, vec3 global_vertex_position) {vec2 mask_a_uv = (uv1 * float(get_layer_map_uv_index(layer, 5) == 0) + uv2 * float(get_layer_map_uv_index(layer, 5) == 1)) * float(layer == 1) + (uv1 * float(get_layer_map_uv_index(layer, 5) == 0) + uv2 * float(get_layer_map_uv_index(layer, 5) == 1)) * float(layer == 2) + (uv1 * float(get_layer_map_uv_index(layer, 5) == 0) + uv2 * float(get_layer_map_uv_index(layer, 5) == 1)) * float(layer == 3);
...
float get_resulting_mask_for_layer(int layer, vec2 uv1, vec2 uv2, vec3 local_surface_normal, vec3 global_surface_normal, vec3 local_vertex_position, vec3 global_vertex_position) {float step_1_mix = get_layer_mask_step_mix(layer, 1, uv1, uv2, local_surface_normal, global_surface_normal, local_vertex_position, global_vertex_position);
...
%layer_fragment_snippets %layer_fragment_snippets 
